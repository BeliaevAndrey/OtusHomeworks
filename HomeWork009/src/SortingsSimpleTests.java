import sortings.BucketSortSeminar;
import sortings.CountingSort;
import sortings.RadixSort;
import util.ArrayGen;
import util.FileWriteSvc;
import util.ViewSvc;

import java.util.HashMap;
import java.util.TreeMap;

public class SortingsSimpleTests {

    private HashMap<Integer, TreeMap<String, Double>> results;
    private int upperLimit;

    public static void main(String[] args) {
        SortingsSimpleTests sst = new SortingsSimpleTests(100_000_000);
        String[] types = new String[]{"Random", "Digits", "Sorted", "Reversed"};
        int[] array;
        App009 app009 = new App009();
        for (String arrayType : types) {
            System.out.println(arrayType);
            sst.results = new HashMap<>();

            for (int i = 1; i <= 1_00_000_000; i *= 10) {
                sst.results.put(i, new TreeMap<>());
                System.out.print("array: " + i);

                array = ArrayGen.getArray(i, arrayType);
                sst.testCountingSort(array, i);
                System.out.print(" Ascending: " + app009.isAscending(array));

                array = ArrayGen.getArray(i, arrayType);
                sst.testBucketSort(array, i);
                System.out.print(" Ascending: " + app009.isAscending(array));

                array = ArrayGen.getArray(i, arrayType);
                sst.testRadixSort(array, i);
                System.out.print(" Ascending: " + app009.isAscending(array));

                System.out.println();
            }
            ViewSvc.printHashMap(sst.results);
            FileWriteSvc.writeHashMap(sst.results, "SimpleTests" + arrayType + ".csv");
        }
    }

    public SortingsSimpleTests(int upperLimit) {
        this.upperLimit = upperLimit;
    }

    void testCountingSort(int[] array, int key) {
        Double end = null;
        System.out.print(" testCounting");
        if (upperLimit > 0 && key <= upperLimit) {
            CountingSort sorter = new CountingSort();
            sorter.init(array);
            long start = System.nanoTime();
            sorter.sort();
            end = (System.nanoTime() - start) / 1e9;
        }
        results.get(key).put("Counting Sort", end);
        System.out.printf(" %e ", end);
    }

    void testBucketSort(int[] array, int key) {
            System.out.print(" testBucketSort");
            BucketSortSeminar sorter = new BucketSortSeminar();
            sorter.init(array);
            long start = System.nanoTime();
            sorter.Bucket();
            Double end = (System.nanoTime() - start) / 1e9;
            results.get(key).put("Bucket Sort", end);
            System.out.printf(" %e ", end);
    }

    void testRadixSort(int[] array, int key) {
        System.out.print(" testRadixSort");
        RadixSort sorter = new RadixSort();
        sorter.init(array);
        long start = System.nanoTime();
        sorter.sort();
        Double end = (System.nanoTime() - start) / 1e9;
        results.get(key).put("Radix Sort", end);
        System.out.printf(" %e ", end);

    }


    public HashMap<Integer, TreeMap<String, Double>> getResults() {
        return results;
    }
}
